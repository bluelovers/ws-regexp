...
class ParserEventEmitter extends EventEmitter {
    constructor(inputAst, flags = '') {
        ...
        if (typeof inputAst == 'string' || inputAst.type == 'Pattern') {
            inputAst = parse_1.fakePatternToRegExpLiteral(inputAst, flags);
        }
    ...
    getSource(overwrite, options) {
        return parse_1.astToString(this.astRegExpLiteral.pattern, Object.assign({}, options, { 
            // @ts-ignore
            debugChanged: overwrite ? 99 : this.astRegExpLiteral.pattern.changed }));
    }
    getFlags(overwrite, options) {
        return parse_1.astToString(this.astRegExpLiteral.flags, Object.assign({}, options, { 
            // @ts-ignore
    ...
    // @ts-ignore
    set source(pattern) {
        pattern = typeof pattern == 'string' ? parse_1.parsePattern(pattern, this.astRegExpLiteral.flags.unicode) : pattern;
    ...
    // @ts-ignore
    set flags(flags) {
        flags = typeof flags == 'string' ? parse_1.parseFlags(flags) : flags;
    ...
    toString(overwrite, options) {
        return parse_1.astToString(this.astRegExpLiteral, Object.assign({}, options, { debugChanged: overwrite ? 99 : this.changed }));
    }
